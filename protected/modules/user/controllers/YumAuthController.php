			...
			if ($success instanceof YumUser) {
				...
				if(Yum::module()->afterLogin !== false) 
					...
					if(Yum::hasModule('forum')){
						Yii::import('application.modules.forum.models.Forumprofile');
						$forumprofile = Forumprofile::model()->find('user_id=:user_id',array(':user_id'=>$success->id));
						$forumprofile->currentLogin= new CDbExpression('NOW()');
						$forumprofile->save();
					}
...
public function actionLogout() {
	...
	if($user = YumUser::model()->findByPk(Yii::app()->user->id)) {
		...
		if(Yum::hasModule('forum')){
		Yii::import('application.modules.forum.models.Forumprofile');
			$forumprofile = Forumprofile::model()->find('user_id=:user_id',array(':user_id'=>Yii::app()->user->id));
			$forumprofile->lastLogin = new CDbExpression('NOW()');
			$forumprofile->save();
		}
