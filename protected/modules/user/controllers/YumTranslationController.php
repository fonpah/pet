...
class YumTranslationController extends YumController
{
	...
	public function actionUpdate($category = null, $message = null, $language = null)
	{
		$models = array();
			foreach(Yum::getAvailableLanguages() as $language) {
				$models[] = $this->loadModelByAttr($category, $message, $language);
			}
	...
	public function loadModelByAttr($category, $message, $language = null)
	{
 ...
 private function loadModelByPk($id){
 	$model =null;
 	if(isset($id))
		$model=YumTranslationController::model()->findbyPk($id);
	if($model===null)
